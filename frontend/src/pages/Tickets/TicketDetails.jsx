...
const TicketDetails = ({ role }) => {
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      ...
      console.error(err);
      alert(err.response?.data?.message || 'You do not have permission to send messages.');
    }
  };
  ...
  if (!ticket) {
    return <div className="text-center p-10 font-bold text-red-500">Ticket not found</div>;
  }
        ...
        <div className="lg:col-span-2 space-y-8">
            ...
            <div className="flex items-start justify-between mb-6 border-b border-slate-100 pb-6">
              <div>
                <div className="flex items-center gap-4 mb-3">
                  <span className="text-sm font-black text-indigo-400 font-mono bg-indigo-50 px-3 py-1 rounded-lg">
                    {ticket.ticketCode}
                  </span>
                  ...
                  {activeRole === 'ADMIN' ? (
                    <select
                      ...
                      className={`px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest border outline-none cursor-pointer ${
              ...
              {canManageTicket && (
                <div className="flex gap-2">
                  <button
                    ...
                    className="p-2.5 bg-slate-50 hover:bg-indigo-50 text-slate-400 hover:text-indigo-600 rounded-xl transition-all"
                    title="Edit Ticket"
                  >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {attachments.map(att => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="block relative group rounded-xl overflow-hidden border border-slate-200 bg-slate-50 aspect-video"
                    >
          ...
          {canViewMessages ? (
              ...
              <div className="px-8 py-5 border-b border-slate-100 flex items-center justify-between bg-white/50 backdrop-blur-md rounded-t-2xl z-10">
                <div>
                  ...
                  <p className="text-xs font-medium text-slate-500 mt-0.5">
                    Messages between reporter and assigned technician
                  </p>
                </div>
              </div>
                ...
                {comments.length === 0 ? (
                  ...
                  comments.map(comment => {
                    ...
                    return (
                      <div
                        key={comment.id}
                        className={`flex gap-2 max-w-[85%] sm:max-w-[70%] w-fit ${
                          isCommentAuthor ? 'ml-auto flex-row-reverse' : 'mr-auto'
                        }`}
                      >
                        {!isCommentAuthor && (
                        ...
                        <div className={`flex flex-col gap-1 ${isCommentAuthor ? 'items-end' : 'items-start'}`}>
                          {!isCommentAuthor && (
                            <div className="flex items-center gap-2 ml-1">
                              <span className="text-slate-400 text-[11px] font-medium">
                                {comment.authorName === ticket.createdBy ? comment.authorName : 'Technician'}
                              </span>

                              {comment.authorName === ticket.createdBy ? (
                                <span className="text-[8px] bg-emerald-100 text-emerald-700 px-1 py-0.5 rounded-sm font-bold uppercase tracking-wider">
                                  Reporter
                                </span>
                              ) : (
                                <span className="text-[8px] bg-amber-100 text-amber-700 px-1 py-0.5 rounded-sm font-bold uppercase tracking-wider">
                                  Tech
                                </span>
                              )}
                            </div>
                          ...
                          <div className={`group relative px-4 py-2 w-fit text-[15px] shadow-sm leading-relaxed ${
                            isCommentAuthor
                              ? 'bg-[#0084ff] text-white rounded-2xl rounded-br-sm'
                              : 'bg-[#e5e5ea] text-black rounded-2xl rounded-bl-sm'
                          }`}>
                            <div className={`absolute top-1/2 -translate-y-1/2 ${
                              isCommentAuthor ? '-left-16' : '-right-16'
                            } opacity-0 group-hover:opacity-100 transition-opacity flex gap-1`}>
                              {isCommentAuthor && editingCommentId !== comment.id && (
                                <button
                                  onClick={() => handleStartEditComment(comment)}
                                  className="p-1.5 bg-white shadow-sm border border-slate-100 rounded-full text-slate-400 hover:text-[#0084ff] transition-colors"
                                  title="Edit"
                                >
                              ...
                              {isCommentAuthor && (
                                <button
                                  ...
                                  className="p-1.5 bg-white shadow-sm border border-slate-100 rounded-full text-slate-400 hover:text-red-500 transition-colors"
                                  title="Delete"
                                >
                            ...
                            {editingCommentId === comment.id ? (
                              <div className="space-y-3 min-w-[200px] sm:min-w-[250px]">
                                <textarea
                                  ...
                                  className="w-full p-2 text-sm outline-none resize-none rounded-xl border bg-white text-black"
                                  rows={2}
                                  autoFocus
                                />
                                ...
                                <div className="flex gap-2 justify-end">
                                  <button
                                    ...
                                    className="px-2 py-1 text-[11px] font-bold rounded-lg text-slate-500 hover:bg-slate-200"
                                  >
                                    Cancel
                                  </button>
                                  ...
                                  <button
                                    ...
                                    className="px-3 py-1 text-[11px] font-bold rounded-lg bg-[#0084ff] text-white"
                                  >
                                    Save
                                  </button>
                                </div>
                ...
                <div className="p-6 bg-white border-t border-slate-100 rounded-b-2xl">
                  <form onSubmit={handleAddComment} className="relative flex items-end gap-3">
                    ...
                    <button
                      ...
                      className="bg-indigo-600 hover:bg-indigo-700 text-white p-4 rounded-2xl disabled:opacity-50 transition-all shadow-sm flex items-center justify-center shrink-0 mb-0.5"
                    >
            ...
            <div className="glass-card p-8 text-center">
              <h3 className="text-lg font-black text-slate-800 mb-2">Discussion Hidden</h3>
              <p className="text-sm font-bold text-slate-500">
                Messages are only visible to the reporter and assigned technician.
              </p>
            </div>
          )}
        </div>
              ...
              <div>
                ...
                <div className="font-medium text-slate-800 flex items-center gap-2">
                  <AlertTriangle className={`w-4 h-4 ${
                    ticket.priority === 'HIGH' || ticket.priority === 'CRITICAL' ? 'text-red-500' : 'text-amber-500'
                  }`} />
                  {ticket.priority}
                </div>
              </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {ticket.assignedTechnician && (
                      <div className="text-xs font-medium text-slate-500 bg-slate-50 p-2 rounded-lg border border-slate-100 flex items-center gap-2">
                        ...
                        <span className="font-bold text-slate-700">{ticket.assignedTechnician}</span>
                      </div>
                    )}
                  </div>
                )}
